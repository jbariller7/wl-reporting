...
/**
 * Build a since/until range using stored ETL cursors.
 * Falls back to a rolling window when no cursor exists.
 *
 * @param {string[]} sources List of sources to consider
 * @param {number} fallbackDays Days to look back when no cursor is present
 */
export async function rangeFromCursor(sources, fallbackDays = 30) {
  const until = dayjs.utc();
  const fallback = until.subtract(fallbackDays, "day");
  let earliest = null;

  for (const source of sources) {
    const cursor = await getCursor(source);
    const candidate = cursor ? dayjs.utc(cursor) : fallback;
    earliest = earliest ? dayjs.min(earliest, candidate) : candidate;
  }
  ...
  return {
    ...
    display: formatDisplay(start, until)
  };
}
