...
/**
 * Fully Corrected Steam Financial ETL
 * Iterates through all App IDs and correctly passes each one to the API.
 */
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  ...
  try {
    ...
    // Filter Steam's returned dates to match your requested range
    ...
    addLog(`Found ${filteredDates.length} dates to sync for ${targetAppIds.length} apps.`);

    // --- STEP 2: Loop through each App and each Date ---
    for (const appId of targetAppIds) {
      addLog(`▶️ Processing Game ID: ${appId}`, 'warn');
      ...
      for (const date of filteredDates) {
        ...
        while (hasMoreForDate) {
          // FIX: We now correctly use the current 'appId' from the loop, not targetAppIds[0]!
          const detailUrl = `${baseUrl.replace(/\/$/, '')}/IPartnerFinancialsService/GetDetailedSales/v1?key=${key}&appid=${appId}&date=${date}&highwatermark_id=${highwatermarkId}`;
          ...
          if (rawData.length > 0) {
            addLog(`✅ SUCCESS: Found ${rawData.length} sales records for App ${appId} on ${date}`, 'success');
    ...
    return { 
      ...
      rows: totalRows, 
      logs, 
      steam_raw_debug: `Finished. Processed ${targetAppIds.length} apps.` 
    };
