...
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  ...
  try {
    ...
    addLog(`Filtered to ${filteredDates.length} dates within your range.`);
    ...
    for (const date of filteredDates) {
      ...
      while (hasMoreForDate) {
        ...
        // --- DEBUG LOG: Tell us what IDs Steam is actually sending ---
        if (rawData.length > 0) {
           const foundIds = [...new Set(rawData.map(r => r.appid))];
           addLog(`Raw data for ${date} contains App IDs: ${foundIds.join(', ')}`);
        }
    ...
    return { ok: true, rows: totalRows, logs, steam_raw_debug: "Steam sync completed." };
  } catch (err) {
