...
/**
 * Final Corrected Meta Ads ETL
 * 1. Handles pagination (loops through ALL pages of data).
 * 2. Increases the limit to 100 rows per request for speed.
 * 3. Correctly maps country-level data to your Google Sheet.
 */
export async function etlMeta({ sinceUtc, untilUtc }) {
  ...
  try {
    addLog(`Querying Meta Insights for account: ${accountId}`);
    ...
    nextUrl.searchParams.set("limit", "100"); // Request 100 rows at a time instead of 25
    ...
    while (nextUrl) {
      ...
      // If Meta provides a 'next' link, we keep looping
      if (json.paging && json.paging.next) {
        nextUrl = json.paging.next;
        addLog(`Found more data... fetching page ${pageCount + 1}`);
      } else {
        nextUrl = null;
      }
    }
    ...
    if (allRows.length > 0) {
      addLog(`✅ Total rows fetched: ${allRows.length}. Sending to Google Sheets...`, 'success');
