...
/**
 * Final Corrected MailerLite ETL
 * Added fallbacks for 'location.country' to capture IP-tracked data.
 */
export async function etlMailerLite({ sinceUtc, untilUtc }) {
  ...
  try {
    addLog("Starting MailerLite sync (with Improved Country tracking)...");
    ...
    while (hasMore && pageCount < 20) {
      ...
      // Debug log for the very first record to see the keys
      if (pageCount === 1) {
        const sample = subs[0];
        addLog(`DEBUG: First sub location=${JSON.stringify(sample.location)}, fields=${JSON.stringify(sample.fields)}`);
      }
      ...
      const batchToUpsert = subs.filter(s => {
        ...
        // FIX: Priority 1: location.country (IP-based), Priority 2: fields.country (Form-based)
        country: s.location?.country || s.fields?.country || s.fields?.Country || s.country || null,
        raw: s
      }));
    ...
    // Quick Group Membership Sync (First 5 groups)
