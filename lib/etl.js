...
export async function etlTelemetry({ sinceUtc, untilUtc, forceRefresh }) {
  ...
  try {
    ...
    // Bulk IP Geolocation translation
    const ipsToLookup = [...new Set(relevantRows.map(r => r.get('ip_address') || r.get('ip') || r.get('IP')).filter(Boolean))];
    let ipToCountryMap = {};
    
    if (ipsToLookup.length > 0) {
      addLog(`Geolocating ${ipsToLookup.length} unique IPs in batches...`);
      
      // api.country.is requires individual GET requests. We batch 50 at a time to prevent overwhelming the server.
      for (let i = 0; i < ipsToLookup.length; i += 50) {
        const chunk = ipsToLookup.slice(i, i + 50);
        await Promise.all(chunk.map(async (ip) => {
          try {
            const geoResp = await fetch(`https://api.country.is/${ip}`);
            if (geoResp.ok) {
              const geoData = await geoResp.json();
              if (geoData.country) ipToCountryMap[ip] = geoData.country;
            }
          } catch (e) {
            // Ignore individual IP fetch errors to keep the batch moving
          }
        }));
      }
    }
    ...
    const batchToUpsert = relevantRows.map(r => {
       const ip = r.get('ip_address') || r.get('ip') || r.get('IP');
       // Use existing country if present, otherwise translated IP, otherwise Unknown
