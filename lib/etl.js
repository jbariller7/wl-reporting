...
/**
 * Ultimate Steam Financial ETL
 * 1. Fetches publisher-wide sales data.
 * 2. Checks multiple possible data keys (data, sales, detailed_sales).
 * 3. Logs every step to identify the exact cause of 0-row transfers.
 */
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  ...
  try {
    ...
    if (filteredDates.length === 0) {
      ...
      return { ok: true, rows: 0, logs, steam_raw_debug: "No changed dates." };
    }

    addLog(`Syncing ${filteredDates.length} dates. Logic: Searching both 'appid' and 'primary_appid'.`);

    // STEP 2: Fetch sales for those dates
    for (const date of filteredDates) {
      addLog(`📅 Querying Steam for date: ${date}`);
      ...
      while (hasMoreForDate) {
        ...
        lastRawResponse = detailJson; // Save for debug display

        const responseObj = detailJson.response || {};
        
        // Steam varies the key name; we check all known possibilities
        const rawData = responseObj.data || responseObj.sales || responseObj.detailed_sales || responseObj.records || [];
        ...
        if (rawData.length > 0) {
          ...
          const relevantRows = rawData.filter(r => {
            ...
            const primaryAppId = String(r.primary_appid || '');
            return targetAppIds.includes(appId) || targetAppIds.includes(primaryAppId);
          });

          if (relevantRows.length > 0) {
            addLog(`✅ FOUND: ${relevantRows.length} rows matched for ${date}`, 'success');
            
            const rowsToUpsert = relevantRows.map(r => ({
              date: date.replace(/\//g, '-'), 
              app_id: String(r.primary_appid || r.appid || r.app_id),
              country: r.country_code || r.country || null,
              currency: r.currency || "USD",
              units: Number(r.gross_units_sold || r.units || 0),
              gross_revenue: parseFloat(r.gross_sales_usd || r.gross_revenue || 0),
              refunds: Number(r.gross_units_returned || r.units_returned || 0),
              net_units: Number(r.net_units_sold || r.net_units || 0),
              net_revenue: parseFloat(r.net_sales_usd || r.net_revenue || 0),
              source: "api",
              raw: r
            }));
            ...
            const sampleIds = rawData.slice(0, 3).map(r => r.appid || r.primary_appid || r.app_id);
            addLog(`ℹ️ Found ${rawData.length} rows for other apps (e.g., ${sampleIds.join(', ')}), but no match for your list.`, 'info');
          }
        } else {
          // If data is missing, log the structure of what Steam DID return
          const keysFound = Object.keys(responseObj).join(', ') || 'None (Empty Response)';
          addLog(`⚠️ Empty data for ${date}. Keys in response: [${keysFound}]`, 'warn');
        }
    ...
    return { 
      ...
      rows: totalRows, 
      logs, 
      steam_raw_debug: lastRawResponse 
    };
