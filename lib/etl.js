...
/**
 * Fetch Steam sales data via Steamworks Sales Data API (if configured) and upsert into steam_sales.
 * Note: The actual API endpoint and response shape may vary.  Adjust as necessary.
 *
 * @param {Object} range Object with sinceUtc and untilUtc ISO strings
 */
/**
 * Fetch Steam sales data via Steamworks Sales Data API and upsert into Google Sheets.
 * Supports multiple App IDs separated by commas in the STEAM_APP_ID environment variable.
 * * @param {Object} range Object with sinceUtc and untilUtc ISO strings
 */
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  const key = process.env.STEAM_PUBLISHER_KEY;
  const apiUrl = process.env.STEAM_SALES_API_URL;
  const appIdsEnv = process.env.STEAM_APP_ID;

  if (!key || !apiUrl || !appIdsEnv) {
    return { ok: false, msg: "Steam Sales API disabled or missing env variables" };
  }

  // Split comma-separated IDs from Netlify env (e.g., "12345, 67890")
  const appIds = appIdsEnv.split(',').map(id => id.trim());
  let totalRows = 0;
  ...
  for (const appId of appIds) {
    try {
      ...
      // Steam strictly requires these exact parameter names in lowercase for the Partner API
      url.searchParams.set("key", key); 
      url.searchParams.set("appid", appId); 
      url.searchParams.set("start_date", sinceUtc.slice(0, 10));
      url.searchParams.set("end_date", untilUtc.slice(0, 10));
      ...
      if (!resp.ok) {
        ...
        return { 
          ok: false, 
          msg: `Steam HTTP ${resp.status} - ${errText.substring(0, 150)}` 
        };
      }
      
      // Capture raw text for debugging and JSON parsing
      const rawText = await resp.text();
      debugResponse = rawText; 
      
      let json;
      try {
        json = JSON.parse(rawText);
      } catch (e) {
        return { ok: false, msg: `Steam gave Invalid JSON: ${rawText.substring(0, 150)}` };
      }

      // Extract data from standard Steam Partner API response paths
      let rawData = json.data || (json.response && json.response.data) || [];
      ...
      const rows = rawData.map((r) => ({
        ...
        refunds: r.refunds || 0,
        net_units: r.net_units || r.units || 0,
      ...
      if (rows.length > 0) {
        // Map the data to the 'steam_sales' table/tab using our Google Sheets upsert
        await upsert(
          "steam_sales",
      ...
      // Catch network timeouts or malformed URLs
      return { ok: false, msg: `Code crashed for App ${appId}: ${err.message}` };
    }
  }
