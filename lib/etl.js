...
export async function etlTelemetry({ sinceUtc, untilUtc, forceRefresh }) {
  ...
  try {
    ...
    let ipToCountryMap = {};
    if (ipsToLookup.length > 0) {
      addLog(`Geolocating ${ipsToLookup.length} unique IPs...`);
      for (let i = 0; i < ipsToLookup.length; i += 100) {
          try {
              const chunk = ipsToLookup.slice(i, i + 100);
              const geoResp = await fetch(`https://api.country.is/`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(chunk) });
              const geoData = await geoResp.json();
              geoData.forEach(res => { if(res.country) ipToCountryMap[res.ip] = res.country; });
          } catch(e) { addLog(`Geo IP error: ${e.message}`, 'warn'); }
      }
    }
