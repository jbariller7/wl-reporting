...
/**
 * Meta Ads ETL with Country Breakdown
 * Requests 'country' breakdown and adds it to the unique key for Google Sheets.
 */
export async function etlMeta({ sinceUtc, untilUtc }) {
  ...
  if (!token || !accountId) {
    return { ok: false, msg: "Missing Meta token or account id", logs };
  }

  try {
    addLog(`Querying Meta Insights with Country breakdown for account: ${accountId}`);
    ...
    const url = new URL(`https://graph.facebook.com/v25.0/${accountId.startsWith('act_') ? accountId : 'act_'+accountId}/insights`);
    ...
    // CRITICAL: This tells Meta to split the data by country
    ...
    if (!resp.ok) {
      const fbError = json.error?.message || "Unknown Meta Error";
      addLog(`Meta API Error (${resp.status}): ${fbError}`, 'error');
      return { ok: false, msg: `Meta ${resp.status}: ${fbError}`, logs };
    }

    const rows = (json.data || []).map((r) => {
      const actions = r.actions || [];
      const purchases = actions.find((a) => a.action_type === "purchase");
      const roas = Array.isArray(r.purchase_roas) && r.purchase_roas[0] ? r.purchase_roas[0].value : null;
      
      return {
        date: r.date_start,
        account_id: String(accountId).replace(/^act_/, ""),
        country: r.country || "Unknown", // Added country mapping
    ...
    if (rows.length > 0) {
      addLog(`✅ Found ${rows.length} country-level Meta rows. Sending to Google Sheets...`, 'success');
      
      // UPDATED Conflict Keys: Includes "country" to allow multiple countries per ad/date
      const { rowCount } = await upsert(
        "meta_insights",
        ["date", "account_id", "ad_id", "country"], 
        ["date","account_id","country","campaign_id","adset_id","ad_id","impressions","clicks","spend","purchases","purchase_value","cpm","cpc","roas","raw","ingested_at"],
        rows
      );
      return { ok: true, rows: rowCount, logs };
    } else {
      addLog("No data found for the selected date range.", 'warn');
      return { ok: true, rows: 0, logs };
    }

  } catch (err) {
    return { ok: false, msg: `Meta Sync Crash: ${err.message}`, logs };
  }
}
