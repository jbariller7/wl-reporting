...
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  ...
  for (const appId of appIds) {
    ...
    // Steam strictly requires these exact parameter names in lowercase
    ...
    const resp = await fetch(url.toString(), {
      method: 'GET' 
      // We removed the headers here because putting the key in the URL 
      // is the most compatible way for Steam's 'IPartnerFinancialService'
    });
      
      // If Steam throws a 404, 401, or 500 error, catch it before it breaks the app!
      if (!resp.ok) {
        const errText = await resp.text();
        return { ok: false, msg: `Steam HTTP ${resp.status} - ${errText.substring(0, 150)}` };
      }
      
      // Download the raw text first so we can debug it if JSON parsing fails
      ...
      // If the URL is malformed or the network times out, catch it here
