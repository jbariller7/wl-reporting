...
/**
 * Campaign-Level Meta Ads ETL
 * Aggregates data by Campaign + Country to avoid row limits and timeouts.
 */
export async function etlMeta({ sinceUtc, untilUtc }) {
  ...
  try {
    addLog(`Querying Meta Insights (Campaign + Country) for: ${accountId}`);

    // Adjusted fields: Removed ad_id/adset_id to focus on Campaign
    const fields = ["date_start","campaign_id","campaign_name","spend","impressions","clicks","purchase_roas","actions"].join(",");
    ...
    // CHANGE 1: Move from 'ad' to 'campaign' level
    ...
    while (nextUrl && pages < 20) { // Safety cap increased to 20 pages (2,000 rows)
      pages++;
    ...
    if (allRows.length > 0) {
      addLog(`✅ Sending ${allRows.length} campaign-level rows to Sheets...`, 'success');
      
      // CHANGE 2: Unique key is now Date + Account + Campaign + Country
      const { rowCount } = await upsert(
        "meta_insights",
        ["date", "account_id", "campaign_id", "country"], 
        ["date","account_id","country","campaign_id","campaign_name","impressions","clicks","spend","purchases","roas","raw","ingested_at"],
        allRows
      );
