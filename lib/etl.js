...
export async function etlStripe({ sinceUtc, untilUtc, forceRefresh }) {
  ...
  if (forceRefresh) {
    ...
    addLog(`Force refresh: Cleared ${delCount} existing rows from Stripe.`);
  }
...
export async function etlMeta({ sinceUtc, untilUtc, forceRefresh }) {
  ...
  try {
    if (forceRefresh) {
      ...
      addLog(`Force refresh: Cleared ${delCount} existing rows from Meta.`);
    }
...
export async function etlTikTok({ sinceUtc, untilUtc, forceRefresh }) {
  ...
  if (forceRefresh) {
    ...
    addLog(`Force refresh: Cleared ${delCount} existing rows from TikTok.`);
  }
...
export async function etlMailerLite({ sinceUtc, untilUtc, forceRefresh }) {
  ...
  if (forceRefresh) {
    ...
    addLog(`Force refresh: Cleared ${delCount} existing rows from MailerLite.`);
  }
...
export async function etlSteamSalesApi({ sinceUtc, untilUtc, forceRefresh }) {
  ...
  if (forceRefresh) {
    ...
    addLog(`Force refresh: Cleared ${delCount} existing rows from Steam.`);
  }
        ...
        if (rawData.length > 0) {
          ...
          if (relevantRows.length > 0) {
            ...
            const rowsToUpsert = relevantRows.map(r => ({
              ...
              refunds: Math.abs(Number(r.gross_units_returned || 0)), 
