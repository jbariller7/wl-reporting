 ...
 */
/**
 * Diagnostic Steam Financial ETL
 * Logs ALL incoming App IDs to identify why filtering is resulting in 0 rows.
 */
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  ...
  try {
    ...
    if (filteredDates.length === 0) {
      return { ok: true, rows: 0, logs, steam_raw_debug: "No financial changes in range." };
    }

    for (const date of filteredDates) {
      addLog(`🔍 Requesting details for: ${date}`);
      let highwatermarkId = 0;
      let hasMoreForDate = true;

      while (hasMoreForDate) {
        const detailUrl = `${baseUrl.replace(/\/$/, '')}/IPartnerFinancialsService/GetDetailedSales/v1?key=${key}&appid=${targetAppIds[0]}&date=${date}&highwatermark_id=${highwatermarkId}`;
        const detailResp = await fetch(detailUrl);
        const detailJson = await detailResp.json();
        
        // Steam response data usually lives in response.data or response.sales
        const rawData = detailJson.response?.data || detailJson.response?.sales || [];
        const maxId = detailJson.response?.max_id || 0;

        if (rawData.length > 0) {
           // DIAGNOSTIC LOG: See exactly what IDs Steam is sending back
           const foundIds = [...new Set(rawData.map(r => r.appid || r.app_id))];
           addLog(`✅ DATA FOUND! Steam reported IDs: ${foundIds.join(', ')} for ${date}`);
           
           // Check for case-insensitive matches or partial matches
           const relevantRows = rawData.filter(r => {
             const id = String(r.appid || r.app_id || '');
           ...
           if (relevantRows.length > 0) {
             const rowsToUpsert = relevantRows.map(r => ({
               ...
               units: Number(r.units_sold || r.units || 0),
             ...
             addLog(`❌ FILTERED: None of those IDs match your STEAM_APP_ID list.`, 'warn');
           }
        } else {
           addLog(`⚠️ Empty response: Steam returned a valid structure but 0 sales records for ${date}.`);
        }
    ...
    return { ok: true, rows: totalRows, logs, steam_raw_debug: "Sync Attempt Finished." };
  } catch (err) {
