...
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  ...
  for (const appId of appIds) {
    ...
    // FIX 1: Send the key exactly how Steam expects it
    url.searchParams.set("key", key); 
    url.searchParams.set("appid", appId); // Steam often prefers 'appid' over 'app_id'
    url.searchParams.set("app_id", appId);
    url.searchParams.set("start_date", sinceUtc.slice(0, 10));
    url.searchParams.set("end_date", untilUtc.slice(0, 10));
    
    const resp = await fetch(url, {
      headers: { 'x-webapi-key': key } // Steam's official header format
    });
    
    const json = await resp.json();
    debugResponse = json; // Capture Steam's exact response to show you on the dashboard!
    
    // Look for the data wherever Steam might be hiding it
  ...
  // Return the raw Steam data back to the webpage so we can read it
  return { 
    ok: true, 
    rows: totalRows,
