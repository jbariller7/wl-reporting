...
/**
 * Ultimate MailerLite ETL with IP Geolocation
 * 1. Checks MailerLite fields/location first.
 * 2. If empty, uses the 'ip_address' to look up the country via api.country.is.
 * 3. Handles both IPv4 and IPv6 automatically.
 */
export async function etlMailerLite({ sinceUtc, untilUtc }) {
  ...
  try {
    addLog("Starting MailerLite sync (with IP Geolocation fallback)...");
    ...
    while (hasMore && pageCount < 20) {
      ...
      // Filter and Map with Geolocation Fallback
      const batchToUpsert = await Promise.all(subs.filter(s => {
        const created = s.created_at || s.subscribed_at;
        return created >= sinceUtc && created <= untilUtc;
      }).map(async (s) => {
        // Step A: Check standard MailerLite fields
        let country = s.location?.country || s.fields?.country || s.fields?.Country || s.country || null;
        
        // Step B: If empty, perform IP Geolocation lookup
        if (!country && s.ip_address) {
          try {
            // We use api.country.is which is free, fast, and supports IPv6
            const geoResp = await fetch(`https://api.country.is/${s.ip_address}`);
            const geoData = await geoResp.json();
            if (geoData.country) {
              country = geoData.country;
            }
          } catch (e) {
            // If geolocation fails, we keep country as null rather than crashing the sync
          }
        }
        ...
        return {
          ...
          country: country, // This will now be populated by the IP lookup!
          raw: s
        };
      }));
    ...
    addLog(`✅ Successfully synced ${totalSynced} subscribers with location tracking.`, 'success');

    // Quick Group Sync (First 5 groups)
