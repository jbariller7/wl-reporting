...
/**
 * Fully Corrected MailerLite ETL
 * 1. Removes the unsupported 'created_at' URL filter.
 * 2. Fetches subscribers using Cursor-based pagination.
 * 3. Filters records by date in JavaScript.
 * 4. Stops fetching once it hits records older than the requested range.
 */
export async function etlMailerLite({ sinceUtc, untilUtc }) {
  ...
  try {
    addLog("Starting MailerLite sync (Fetch & Local Filter logic)...");
    ...
    // STEP 2: Fetch and Filter Subscribers
    let nextCursor = null;
    let totalSynced = 0;
    let hasMore = true;
    let pageCount = 0;

    addLog(`Scanning subscribers for range: ${sinceUtc.slice(0,10)} to ${untilUtc.slice(0,10)}`);

    while (hasMore && pageCount < 20) { // Safety cap at 2,000 subscribers
      pageCount++;
      
      // Use cursor-based URL as required by the New API
      ...
      // Local Filtering: Check if subscriber was created within our range
      const batchToUpsert = subs.filter(s => {
      ...
      // Optimization: If the oldest sub in this batch is already older than our 'sinceUtc', we can stop
      ...
      if (oldestInBatch && oldestInBatch < sinceUtc) {
        addLog(`Reached records older than ${sinceUtc.slice(0,10)}. Stopping scan.`);
        hasMore = false;
      } else {
        nextCursor = data.meta?.next_cursor || null;
        hasMore = !!nextCursor;
        if (hasMore) addLog(`Fetched page ${pageCount}, continuing scan...`);
      }
    }

    addLog(`✅ Successfully synced ${totalSynced} matching subscribers.`, 'success');

    // STEP 3: Quick Membership Update (First 5 groups)
