 ...
 * 1. Fetches all publisher sales for a date (without broken URL filters).
 * 2. Matches against App IDs and Primary App IDs returned by Steam.
 */
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  ...
  try {
    // STEP 1: Get the list of dates Steam has data for
    ...
    if (filteredDates.length === 0) {
      addLog("No financial changes found in this date range.");
      return { ok: true, rows: 0, logs };
    }

    addLog(`Syncing ${filteredDates.length} dates for ${targetAppIds.length} apps...`);

    // STEP 2: Fetch all sales for those dates
    for (const date of filteredDates) {
      addLog(`📅 Requesting all publisher sales for: ${date}`);
      let highwatermarkId = 0;
      let hasMoreForDate = true;

      while (hasMoreForDate) {
        // IMPORTANT: No &appid= in this URL. We fetch everything and filter in JS.
        ...
        if (rawData.length > 0) {
          // Identify if any of the rows belong to your games
          ...
          if (relevantRows.length > 0) {
            addLog(`✅ SUCCESS: Found ${relevantRows.length} matching sales for your games on ${date}`, 'success');
            ...
            const rowsToUpsert = relevantRows.map(r => ({
              ...
              refunds: Number(r.gross_units_returned || 0),
            ...
            // Log what IDs were found so you can see if you missed any
            const otherIds = [...new Set(rawData.map(r => r.primary_appid || r.appid))];
            addLog(`ℹ️ Found sales for other IDs (${otherIds.join(', ')}), but none matched your list.`, 'info');
          }
        }
    ...
    return { ok: true, rows: totalRows, logs, steam_raw_debug: "Sync complete." };
