...
import { archiveRange } from "./blob-store.js";
...
export async function etlStripe({ sinceUtc, untilUtc }) {
  ...
  while (more) {
    ...
    const archiveBatch = batch.map(({ raw, ...rest }) => rest);
    archivePayload.push(...archiveBatch);
    await upsert(
      "stripe_orders",
      ["id"],
      [
  ...
  const archiveKey = await archiveRange("stripe", { sinceUtc, untilUtc }, archivePayload);
  return { ok: true, rows: total, archiveKey };
}
...
export async function etlMeta({ sinceUtc, untilUtc }) {
  ...
  const archiveRows = rows.map(({ raw, ...rest }) => rest);
  const archiveKey = await archiveRange("meta", { sinceUtc, untilUtc }, archiveRows);
  return { ok: true, rows: rows.length, archiveKey };
}
...
export async function etlTikTok({ sinceUtc, untilUtc }) {
  ...
  const archiveRows = rows.map(({ raw, ...rest }) => rest);
  const archiveKey = await archiveRange("tiktok", { sinceUtc, untilUtc }, archiveRows);
  return { ok: true, rows: rows.length, archiveKey };
}
...
export async function etlMailerLite({ sinceUtc, untilUtc }) {
  ...
  const archiveKey = await archiveRange("mailerlite", { sinceUtc, untilUtc }, archiveSubs);
  return { ok: true, rows: total, archiveKey };
}
...
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  ...
  if (!key || !apiUrl || !appId)
    return { ok: false, msg: "Steam Sales API disabled or missing env" };
  ...
  if (!resp.ok) throw new Error(`Steam Sales ${resp.status}`);
  const json = await resp.json();
  const rows = (json.data || []).map((r) => ({
    date: r.date,
    app_id: String(appId),
  ...
  const archiveRows = rows.map(({ raw, ...rest }) => rest);
  const archiveKey = await archiveRange("steam", { sinceUtc, untilUtc }, archiveRows);
  return { ok: true, rows: rows.length, archiveKey };
}