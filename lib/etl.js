...
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  ...
  // Split the comma-separated list of IDs from Netlify (e.g., "123450, 543210")
  ...
  for (const appId of appIds) {
    ...
    if (!resp.ok) {
      console.error(`Steam Sales API failed for App ID ${appId}: ${resp.status}`);
      continue; // If one game fails, skip it and try the next one
    }
