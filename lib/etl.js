...
/**
 * Publisher-Wide Steam Financial ETL
 * Fetches all sales data for the account and filters by your target App IDs in code.
 */
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  ...
  try {
    ...
    if (filteredDates.length === 0) {
      addLog("No financial changes found in the requested date range.");
      return { ok: true, rows: 0, logs, steam_raw_debug: "Done." };
    }

    addLog(`Syncing ${filteredDates.length} dates for your publisher account...`);

    // --- STEP 2: Fetch ALL sales for these dates (No AppID filter in URL) ---
    for (const date of filteredDates) {
      ...
      while (hasMoreForDate) {
        // We REMOVED the &appid= parameter to pull all publisher data at once
        ...
        // Steam data structure check
        const responseObj = detailJson.response || {};
        const rawData = responseObj.data || responseObj.sales || [];
        const maxId = responseObj.max_id || 0;

        // CRITICAL DEBUG: If we get an empty response object, log it
        if (Object.keys(responseObj).length === 0) {
           addLog(`⚠️ Steam returned an empty response object for ${date}. This usually means no records exist or parameters are wrong.`, 'warn');
        }

        if (rawData.length > 0) {
          // Identify what apps are actually in the data
          const foundIds = [...new Set(rawData.map(r => String(r.appid || r.app_id || 'unknown')))];
          addLog(`✅ DATA FOUND! Records found for App IDs: ${foundIds.join(', ')}`, 'success');
          
          const relevantRows = rawData.filter(r => targetAppIds.includes(String(r.appid || r.app_id)));
          
          if (relevantRows.length > 0) {
            const rowsToUpsert = relevantRows.map(r => ({
              date: date.replace(/\//g, '-'), 
              app_id: String(r.appid || r.app_id),
              ...
              refunds: Number(r.units_returned || 0),
            ...
            await upsert("steam_sales", ["date", "app_id", "country", "currency"], 
            ...
            totalRows += rowsToUpsert.length;
             ...
             addLog(`ℹ️ Skipping ${rawData.length} rows because they don't match your target App IDs.`, 'info');
          }
        }
