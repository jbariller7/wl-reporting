...
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  ...
  const baseUrl = process.env.STEAM_SALES_API_URL; // Should be https://partner.steam-api.com
  ...
  try {
    // --- STEP 1: Get Changed Dates ---
    addLog("Checking Steam for changed financial dates...");
    const datesUrl = new URL(`${baseUrl}/IPartnerFinancialsService/GetChangedDatesForPartner/v1/`);
    datesUrl.searchParams.set("key", key);
    datesUrl.searchParams.set("highwatermark", "0"); // 0 pulls all available changes for your range

    const datesResp = await fetch(datesUrl.toString());
    const datesJson = await datesResp.json();

    if (!datesResp.ok || !datesJson.response) {
      return { ok: false, msg: `Steam Dates API Error: ${JSON.stringify(datesJson)}` };
    }

    const changedDates = datesJson.response.dates || [];
    addLog(`Found ${changedDates.length} dates with financial changes.`);

    if (changedDates.length === 0) {
      return { ok: true, rows: 0, steam_raw_debug: "No changed dates returned by Steam." };
    }

    // --- STEP 2: Fetch Details for each Date ---
    for (const date of changedDates) {
      addLog(`Processing date: ${date}`);
      let highwatermarkId = 0;
      let hasMoreForDate = true;

      while (hasMoreForDate) {
        const detailUrl = new URL(`${baseUrl}/IPartnerFinancialsService/GetDetailedSales/v1/`);
        detailUrl.searchParams.set("key", key);
        detailUrl.searchParams.set("date", date);
        detailUrl.searchParams.set("highwatermark_id", highwatermarkId.toString());

        const detailResp = await fetch(detailUrl.toString());
        const detailJson = await detailResp.json();

        if (!detailResp.ok || !detailJson.response) {
           addLog(`Error fetching details for ${date}: ${JSON.stringify(detailJson)}`, 'error');
           break;
        }

        const rawData = detailJson.response.data || [];
        const maxId = detailJson.response.max_id || 0;

        // Filter data for your specific App IDs
        const filteredRows = rawData.filter(r => targetAppIds.includes(String(r.appid)));
        
        if (filteredRows.length > 0) {
          const rowsToUpsert = filteredRows.map(r => ({
            ...
            refunds: 0, // This API separates refunds into their own rows usually
            net_units: Number(r.units_sold || 0),
            net_revenue: parseFloat(r.gross_sales_usd || 0),
            source: "api",
            raw: r
          }));
          ...
          await upsert(
            "steam_sales",
            ["date", "app_id", "country", "currency"],
            [
              "date", "app_id", "country", "currency", "units",
              "gross_revenue", "refunds", "net_units", "net_revenue",
              "source", "raw", "ingested_at"
            ],
            rowsToUpsert
          );
          totalRows += rowsToUpsert.length;
        }

        // Pagination check: Stop if max_id didn't move forward
    ...
    return { 
      ...
      rows: totalRows, 
      logs,
      steam_raw_debug: "Financial sync complete." 
    };

  } catch (err) {
    return { ok: false, msg: `Steam ETL Error: ${err.message}`, logs };
  }
}
