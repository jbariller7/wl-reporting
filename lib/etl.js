...
export async function etlSteamSalesApi({ sinceUtc, untilUtc }) {
  ...
  try {
    ...
    addLog(`Checking Steam for changed dates...`);
    
    const datesResp = await fetch(datesUrl);
    
    if (!datesResp.ok) {
      const errBody = await datesResp.text();
      return { ok: false, msg: `Steam Dates API Error ${datesResp.status}: ${errBody.substring(0, 200)}`, logs };
    }

    const datesJson = await datesResp.json();
    const changedDates = datesJson.response?.dates || [];
    addLog(`Steam found ${changedDates.length} total dates with financial updates.`);
    ...
    addLog(`Filtered to ${filteredDates.length} dates within range.`);

    if (filteredDates.length === 0) {
      return { ok: true, rows: 0, logs, steam_raw_debug: "No financial changes found." };
    }

    for (const date of filteredDates) {
      addLog(`Syncing date: ${date}`);
      ...
      while (hasMoreForDate) {
        ...
        if (!detailResp.ok) {
           addLog(`Error for ${date}: HTTP ${detailResp.status}`, 'error');
           break;
        }
        ...
        const relevantRows = rawData.filter(r => targetAppIds.includes(String(r.appid)));
    ...
    return { 
      ...
      rows: totalRows, 
      logs,
      steam_raw_debug: "Steam sync completed successfully." 
    };

  } catch (err) {
    return { ok: false, msg: `Steam ETL Fatal Error: ${err.message}`, logs };
  }
}
