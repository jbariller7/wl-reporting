...
/**
 * Optimized Meta Ads ETL
 * 1. Catch HTML errors to prevent "Unexpected token <" crashes.
 * 2. Optimized fields to reduce data payload size.
 * 3. Added a safety break to prevent infinite loops during timeouts.
 */
export async function etlMeta({ sinceUtc, untilUtc }) {
  ...
  try {
    addLog(`Querying Meta Insights (Country Breakdown) for: ${accountId}`);

    const fields = ["date_start","ad_id","adset_id","campaign_id","spend","impressions","clicks","purchase_roas","actions"].join(",");
    ...
    nextUrl.searchParams.set("limit", "50"); // Reduced slightly to avoid huge payloads

    let allRows = [];
    let pages = 0;

    while (nextUrl && pages < 10) { // Safety cap at 10 pages to avoid Netlify timeout
      pages++;
      ...
      // --- NEW: Check if response is actually JSON before parsing ---
      const contentType = resp.headers.get("content-type");
      if (!contentType || !contentType.includes("application/json")) {
        const text = await resp.text();
        addLog(`⚠️ Server returned non-JSON response (likely a timeout). Processing what we have so far.`, 'warn');
        break; 
      }
    ...
    if (allRows.length > 0) {
      addLog(`✅ Sending ${allRows.length} rows to Sheets...`, 'success');
      const { rowCount } = await upsert(
        "meta_insights",
        ["date", "account_id", "ad_id", "country"], 
        ["date","account_id","country","campaign_id","adset_id","ad_id","impressions","clicks","spend","purchases","roas","raw","ingested_at"],
        allRows
      );
