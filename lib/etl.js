...
/**
 * Robust MailerLite ETL
 * Supports the new MailerLite API (connect.mailerlite.com).
 * Includes detailed logging and error catching to prevent dashboard crashes.
 */
export async function etlMailerLite({ sinceUtc, untilUtc }) {
  ...
  try {
    addLog("Starting MailerLite sync...");

    // STEP 1: Fetch Groups (to track memberships)
    addLog("Fetching groups list...");
    const groupsResp = await fetch("https://connect.mailerlite.com/api/groups", { headers });
    
    if (!groupsResp.ok) {
      const errBody = await groupsResp.text();
      addLog(`MailerLite API Error (${groupsResp.status}): ${errBody}`, 'error');
      return { ok: false, msg: `MailerLite Error ${groupsResp.status}`, logs };
    }
    
    const groupsData = await groupsResp.json();
    const groups = groupsData.data || [];
    addLog(`Found ${groups.length} groups.`);

    // STEP 2: Fetch Subscribers in range
    let page = 1;
    let totalSubs = 0;
    let hasMore = true;

    addLog(`Fetching subscribers created between ${sinceUtc.slice(0, 10)} and ${untilUtc.slice(0, 10)}...`);

    while (hasMore) {
      const subUrl = `https://connect.mailerlite.com/api/subscribers?limit=100&page=${page}&filter[created_at][from]=${sinceUtc}&filter[created_at][to]=${untilUtc}`;
      const resp = await fetch(subUrl, { headers });
      const data = await resp.json();

      if (!resp.ok) throw new Error(data.message || `Subscribers API Error ${resp.status}`);

      const subs = data.data || [];
      if (subs.length === 0) break;

      addLog(`Processing page ${page} (${subs.length} subscribers)...`);
      ...
      const batch = subs.map((s) => ({
        ...
        country: s.fields?.country || s.country || null,
        raw: s
      }));
      ...
      totalSubs += batch.length;
      
      // Check for next page
      if (data.links?.next && page < 10) { // Safety cap at 10 pages
        page++;
      } else {
        hasMore = false;
      }
    ...
    addLog(`✅ Successfully synced ${totalSubs} subscribers.`, 'success');

    // STEP 3: Update Group Memberships (Optional Snapshot)
    // To save time and avoid Netlify timeouts, we only process the first 5 groups
    for (const g of groups.slice(0, 5)) {
      addLog(`Syncing memberships for group: ${g.name}`);
      ...
      const gSubs = gData.data || [];
      
      const membershipBatch = gSubs.map(s => ({
        subscriber_id: String(s.id),
    ...
    return { ok: true, rows: totalSubs, logs };

  } catch (err) {
    return { ok: false, msg: `MailerLite Sync Crash: ${err.message}`, logs };
  }
}
