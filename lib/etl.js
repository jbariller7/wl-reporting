...
/**
 * Robust Meta Ads ETL
 * Catches API errors and logs the specific reason for failure.
 */
export async function etlMeta({ sinceUtc, untilUtc }) {
  ...
  try {
    ...
    // Marketing API v23.0 is valid, but v25.0 is the latest as of Feb 2026
    const fields = ["date_start","spend","impressions","clicks","purchase_roas","actions"].join(",");
    ...
    if (rows.length > 0) {
      addLog(`✅ Found ${rows.length} Meta insight rows. Sending to Google Sheets...`, 'success');
      const { rowCount } = await upsert(
        "meta_insights",
        ["date", "account_id", "ad_id"],
        ["date","account_id","campaign_id","adset_id","ad_id","impressions","clicks","spend","purchases","purchase_value","cpm","cpc","roas","raw","ingested_at"],
        rows
      );
