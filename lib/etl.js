...
/**
 * Robust Meta Ads ETL
 * Fixed: Added ad_id to fields to prevent deduplication collisions.
 */
export async function etlMeta({ sinceUtc, untilUtc }) {
  ...
  try {
    ...
    // FIX: Added ad_id, campaign_id, and adset_id to the fields list
    const fields = [
      "date_start",
      "ad_id",
      "adset_id",
    ...
    if (rows.length > 0) {
      addLog(`✅ Found ${rows.length} unique Meta rows. Sending to Google Sheets...`, 'success');
