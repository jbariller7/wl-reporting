...
export async function clearDateRange(table, dateCol, since, until) {
  ...
  if (requests.length > 0) {
    try {
      ...
      // CRITICAL FIX: Destroy the local document cache.
      // This forces the immediately following `upsert` function to re-download 
      // the sheet data directly from Google, ensuring the old rows are gone.
      docPromise = null;
      
    } catch (e) {
      console.error(`Failed to bulk delete rows in ${tabName}:`, e.message);
    }
  }
